...
export class AuthService {
  ...
  static async updateEmployeeProfile(token: string, empId: number, data: ProfileUpdateData): Promise<ProfileUpdateResponse> {
    try {
      const headers = this.getAuthHeaders(token)
      const response = await fetch(`${API_CONFIG.baseURL}${API_ENDPOINTS.employee.editProfile}/${empId}`, {
        method: 'PATCH',
        headers,
